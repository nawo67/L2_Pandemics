...
public class ScientistFindRemedyAction extends FindRemedyAction {
    /**
     * Constructor for the find remedy action specific to the scientist role 
     */
    public ScientistFindRemedyAction(){};
        ...
        if (!p.getLocation().hasLab()){throw new Exception(" Can't find remedy without a lab");}
            ...
            //int count = 0;
             ...
             for (Disease d : diseases){
                if (this.countCardsForDisease(d, p).size() >= 4){
                    d.setRemedy();
                    int count = 0;
